        ...
        <ul class="metismenu list-unstyled" id="side-menu">
          <template v-for="item in menuItems">
            ...
            $t(item.badge.text) }}</span>
              ...
              <router-link :to="item.link" v-if="!hasItems(item) && can(item.permission)" class="side-nav-link-ref">
                <i :class="`bx ${item.icon}`" v-if="item.icon"></i>
                <span>{{ $t(item.label) }}</span>
                <span :class="`badge rounded-pill bg-${item.badge.variant} float-end`" v-if="item.badge">{{
            $t(item.badge.text) }}</span>
              ...
              <ul v-if="hasItems(item)" class="sub-menu" aria-expanded="false">
                <li v-for="(subitem, index) of item.subItems" :key="index">
                  ...
                  <router-link :to="subitem.link" v-if="!hasItems(subitem) && can(subitem.permission)"
                    class="side-nav-link-ref">
                    {{ $t(subitem.label) }}
                  </router-link>

                  <a v-if="hasItems(subitem) && can(subitem.permission)" class="side-nav-link-a-ref has-arrow"
